...
void Foam::dropletCloud::info()
{
    ...
    if (cloudSize > 0)
    {
        Info<< "    Max diameter [um]  : " << 1e6*Dmax() << endl
            << "    D10 diameter [um]  : " << 1e6*Dij(1,0) << endl
            << "    D32 diameter [um]  : " << 1e6*Dij(3,2) << endl;
    }
    Info<< endl;
}
    ...
    breakup_(mesh_)
{
    ...
    const dictionary& thermophysicalProperties = db().lookupObject<IOdictionary>
    (
    ...
    // surface tension
    rhop_ =  readScalar(thermophysicalProperties.subDict("mixture").subDict("equationOfState").lookup("rho0"));
    // dynamic viscosity
    mup_  =  readScalar(thermophysicalProperties.subDict("mixture").subDict("transport").lookup("mu"));
    
    const dictionary& thermophysicalProperties1 = db().lookupObject<IOdictionary>
    (
    ...
    // surface tension
    sigma_ = readScalar(thermophysicalProperties1.lookup("sigma"));
...
void Foam::dropletCloud::move()
{
   ...
   // All domain mixture phase properties
    const volScalarField& rho = mesh_.lookupObject<const volScalarField>("rho");
    const volVectorField& U = mesh_.lookupObject<const volVectorField>("U");
    const volScalarField& mu = mesh_.lookupObject<const volScalarField>("thermo:mu");

    interpolationCellPoint<scalar> rhoInterp(rho);
    interpolationCellPoint<vector> UInterp(U);
    interpolationCellPoint<scalar> muInterp(mu);
    ...
    droplet::trackingData
        td(*this, rhoInterp, UInterp, muInterp, g_.value());
